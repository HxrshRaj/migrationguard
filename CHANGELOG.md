...
## 18. LatentStack as a second advanced-mode provider

*Drafted by the LatentStack coding agent on a scoped task; reviewed and verified here — the agent placed this entry at the top of the changelog rather than in build order, and left `cli.py`'s import block unsorted (`ruff` `I001`); both fixed.*

`migrationguard/llm.py` had exactly one real provider (`AnthropicLLMClient`). `LatentStackLLMClient` is a second, structurally identical implementation of the `LLMClient` protocol that talks to the LatentStack gateway (`https://latentstack.dev/v1`, OpenAI-compatible Chat Completions, default model `gemini/gemini-3.1-pro`) via the `openai` SDK — same bounded exponential backoff, same retry/degrade/raise classification (mapped onto `openai`'s exception hierarchy), same `failures` list, same `<CALL FAILED …>` degradation and trajectory logging. `cli.py` gains `--provider [anthropic|latentstack]` (default `anthropic`, so nothing changes unless asked); `cost.py` gets a `gemini/gemini-3.1-pro` price row (placeholder rate, flagged `TODO`); `openai>=1.40,<2` added as a dependency.

**Not in this entry:** a live end-to-end advanced run on the gateway, and the real cost/latency numbers from it. This PR is the wiring + unit coverage only; the live run and its `trajectories.jsonl` are captured separately (see `REPRODUCTION.md` §4b).

**Evidence:** 5 new tests in `tests/test_llm_latentstack.py`, mirroring `test_llm_retry.py` against an injected fake transport (no network, no key): succeeds after transient failures with the right number of backoff sleeps and a clean trajectory line; degrades to `""` after `max_attempts` with a `<CALL FAILED …>` line and a recorded failure; a non-retryable verdict degrades on the first attempt with no sleep; an auth verdict raises `LLMCallError`; and `_classify()` maps real `openai.APITimeoutError` / `APIConnectionError` / `RateLimitError` / `InternalServerError` / `BadRequestError` / `AuthenticationError` / `PermissionDeniedError` instances to retry / retry / retry / retry / degrade / raise / raise. Full suite: 87 passed; `ruff check .` and `mypy` clean.

## 19. First real advanced-mode run — on the LatentStack gateway

The advanced-mode numbers quoted throughout this repo had, until now, all come from the `--fake-llm` dry run — no real key was available during the improvement pass. This entry replaces them with a real run: `migrationguard scan --mode advanced --provider latentstack` (from entry 18) against `gemini/gemini-3.1-pro` on the LatentStack gateway, `--max-examples 150 --seed 20260830`, on 2026-08-29.

One fix landed first: probing the gateway showed Gemini 3.1 Pro is a reasoning model that spends part of its token budget on internal reasoning before emitting any content — at the Anthropic client's `max_tokens=1536` a full-function fix rewrite came back empty. `LatentStackLLMClient`'s ceiling was raised to 4096.

**Result:** 7 findings, **7/7 auto-fixed** (5 template, 2 via the LLM fixer — `search_users_by_email_domain` and `list_active_users_over_age`, the two the template provably can't touch). 683 verification cases → **473 identical / 0 cosmetic / 210 breaking**, every breaking case on adversarial input, matching the intended story. **39 disclosed LLM calls** (7 explanation + 2 fix + 30 rationale), **0 failed or degraded**. 8,292 input + 29,894 output tokens; **~$0.31** at the placeholder rate (the token counts are exact, the rate isn't confirmed). Wall-clock **~7 min 9 s** — 4.5 s min / 10.6 s mean / 19.5 s max per call, the reasoning model being the entire cost.

**Evidence / disclosure artifacts** (committed under `artifacts/`, since `out/` is git-ignored): `trajectories-latentstack.jsonl` — the full 39-call disclosure log, every entry `"model": "gemini/gemini-3.1-pro"`, prompt + response + tokens + latency; `report-advanced-latentstack.html` — the rendered report; `run-latentstack.jsonl` — the structured stage log. `REPRODUCTION.md` §4b carries the same numbers in a table.
